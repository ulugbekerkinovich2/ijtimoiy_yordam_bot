...
#     await Form.message.set()  # Keyingi holatga o'ting - telefon raqami
...
class Form(StatesGroup):
    name = State()  # Ism va familiya holati
    phone = State()
    category = State()  # Tanlangan kategoriya holati
    message = State()  # Foydalanuvchi xabarini qabul qilish holati

# categories = {
#     1: "Ёлғиз кекса деб эътироф этиш (ёки бекор қилиш)",
#     2: "Чуқурлаштирилган тиббий кўрикдан ўтказиш",
#     3: "Ҳужжатларини расмийлаштириш ва қайта тиклашда ёрдам бериш",
#     # Qo'shimcha kategoriyalar qisqartirilган.
# }
...
async def ask_for_full_name(message: types.Message, state: FSMContext):
    await state.update_data(full_name=message.text)
    await Form.phone.set()  # Keyingi holatga o'ting - telefon raqami
    contact_keyboard = ReplyKeyboardMarkup(resize_keyboard=True, one_time_keyboard=True)
    contact_button = KeyboardButton("Telefon raqamni yuborish 📞", request_contact=True)
    contact_keyboard.add(contact_button)
    await message.answer("Iltimos, telefon raqamingizni yuboring:", reply_markup=contact_keyboard)
...
async def ask_for_assistance_type(message: types.Message, state: FSMContext):
    await state.update_data(phone_number=message.contact.phone_number)
    await Form.category.set()  # Holatni "category"ga o'zgartiramiz
    await message.answer("Sizga qanday ijtimoий yordam kerak? Kategoriyalarni tanlash uchun quyidagi ro'yxatdan foydalaning.", reply_markup=await get_main_keyboard())
...
async def receive_user_message(message: types.Message, state: FSMContext):
    ...
    await bot.send_message(chat_id='-1002482460312', text=text_to_send, parse_mode='HTML')

    # Foydalanuvchiga tasdiq yuborish
    await message.answer("Ma'lumotlaringiz qabul qilindi. Botdan foydalanganingiz uchun tashakkur!", reply_markup=ReplyKeyboardRemove())

    # Holatni tozalash
    await state.finish()

@dp.callback_query_handler(lambda c: c.data and c.data.startswith('page_'), state=Form.category)
